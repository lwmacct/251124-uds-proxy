...
- [启动代理](#启动代理) `:50+7`
- [基础操作](#基础操作) `:57+34`
  - [获取 Docker 版本](#获取-docker-版本) `:59+20`
  - [获取系统信息](#获取系统信息) `:79+6`
  - [检查 Docker 连通性](#检查-docker-连通性) `:85+6`
- [容器操作](#容器操作) `:91+74`
  - [列出所有容器](#列出所有容器) `:93+13`
  - [创建容器](#创建容器) `:106+18`
  - [启动容器](#启动容器) `:124+7`
...
- [数据卷操作](#数据卷操作) `:217+17`
  ...
  - [创建数据卷](#创建数据卷) `:225+9`
- [错误处理](#错误处理) `:234+32`
  - [Socket 不存在](#socket-不存在) `:236+7`
  - [权限不足](#权限不足) `:243+10`
  - [判断错误类型](#判断错误类型) `:253+13`
- [实用脚本](#实用脚本) `:266+30`
  - [批量清理停止的容器](#批量清理停止的容器) `:268+15`
  - [监控容器状态](#监控容器状态) `:283+13`
