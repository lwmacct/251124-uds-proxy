...
- [目录结构](#目录结构) `:20+19`
- [核心模块](#核心模块) `:39+108`
  - [1. 配置管理 (config.go)](#1-配置管理-configgo) `:41+16`
  - [2. HTTP 服务器 (server.go)](#2-http-服务器-servergo) `:57+15`
  - [3. 请求处理器 (handlers.go)](#3-请求处理器-handlersgo) `:72+26`
  - [4. 连接池 (pool.go)](#4-连接池-poolgo) `:98+41`
  - [5. CLI 命令 (command.go)](#5-cli-命令-commandgo) `:139+8`
- [错误处理](#错误处理) `:147+13`
- [技术栈](#技术栈) `:160+6`
